package com.example.mapnavigation.overlay;
...
public class BusRouteOverlay extends RouteOverlay {
		...
		try {
			...
			for (int i = 0; i < busSteps.size(); i++) {
				...
				if (busStep.getBusLine() != null) {
					...
					if (i == busSteps.size() - 1) {
						addWalkPolyline(AMapUtils.convertToLatLng(getLastBuslinePoint(busStep)), endPoint);
					}
				}
				if (busStep.getRailway() != null) {
					...
					if (i == busSteps.size() - 1) {
						addWalkPolyline(AMapUtils.convertToLatLng(busStep.getRailway().getArrivalstop().getLocation()), endPoint);
					}
				}
	...
	private void addRailwayStep(RouteRailwayItem railway) {
		...
		for (int i = 0; i < railwayStationItems.size(); i++) {
			railwaylistpoint.add(AMapUtils.convertToLatLng(railwayStationItems.get(i).getLocation()));
		}
	...
	private void addTaxiStep(TaxiItem taxi){
		addPolyLine(new PolylineOptions().width(getRouteWidth())
				.color(getBusColor())
				.add(AMapUtils.convertToLatLng(taxi.getOrigin()))
				.add(AMapUtils.convertToLatLng(taxi.getDestination())));
	}
	...
	private void addWalkSteps(BusStep busStep) {
		...
		for (int j = 0; j < walkSteps.size(); j++) {
			...
			if (j == 0) {
				LatLng latLng = AMapUtils.convertToLatLng(walkStep
						.getPolyline().get(0));
			...
			List<LatLng> listWalkPolyline = AMapUtils
			...
			if (j < walkSteps.size() - 1) {
				LatLng lastLatLng = listWalkPolyline.get(listWalkPolyline
						.size() - 1);
				LatLng firstlatLatLng = AMapUtils
	...
	private void addBusLineSteps(List<LatLonPoint> listPoints) {
		...
		addPolyLine(new PolylineOptions().width(getRouteWidth())
				.color(getBusColor())
				.addAll(AMapUtils.convertArrList(listPoints)));
	}
	...
	private void addBusStationMarkers(RouteBusLineItem routeBusLineItem) {
		...
		LatLng position = AMapUtils.convertToLatLng(startBusStation
				.getLatLonPoint());
	...
	private void addTaxiMarkers(TaxiItem taxiItem) {
		...
		LatLng position = AMapUtils.convertToLatLng(taxiItem
				.getOrigin());
	...
	private void addRailwayMarkers(RouteRailwayItem railway) {
		LatLng Departureposition = AMapUtils.convertToLatLng(railway
		...
		LatLng Arrivalposition = AMapUtils.convertToLatLng(railway
	...
	private void checkBusToNextBusNoWalk(BusStep busStep, BusStep busStep1) {
		LatLng endbusLatLng = AMapUtils
				.convertToLatLng(getLastBuslinePoint(busStep));
		LatLng startbusLatLng = AMapUtils
	...
	private void checkBusEndToNextBusStart(BusStep busStep, BusStep busStep1) {
		LatLonPoint busLastPoint = getLastBuslinePoint(busStep);
		LatLng endbusLatLng = AMapUtils.convertToLatLng(busLastPoint);
		LatLonPoint busFirstPoint = getFirstBuslinePoint(busStep1);
		LatLng startbusLatLng = AMapUtils.convertToLatLng(busFirstPoint);
	...
	private void addWalkPolyLineByLatLonPoints(LatLonPoint pointFrom,
			LatLonPoint pointTo) {
		LatLng latLngFrom = AMapUtils.convertToLatLng(pointFrom);
		LatLng latLngTo = AMapUtils.convertToLatLng(pointTo);
