...
public class DrivingRouteOverlay extends RouteOverlay{
    ...
    private void colorWayUpdate(List<TMC> tmcSection) {
        ...
        mPolylineOptionscolor.add(AMapUtils.convertToLatLng(tmcSection.get(0).getPolyline().get(0)));
        ...
        for (int i = 0; i < tmcSection.size(); i++) {
			...
			for (int j = 1; j < mployline.size(); j++) {
				mPolylineOptionscolor.add(AMapUtils.convertToLatLng(mployline.get(j)));
				colorList.add(color);
			}
		}
